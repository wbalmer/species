...
class AtmosphericRetrieval:
    ...
    def run_dynesty(
        ...
        if "n_live_points" in multinest_kwargs:
            ...
            if not hasattr(self, "n_live_points"):
                n_live_points = 2000
        if "npool" in multinest_kwargs:
            npool = multinest_kwargs["npool"]
        else:
            npool = 4
        if "sample_method" in multinest_kwargs:
            sample_method = multinest_kwargs["sample_method"]
        else:
        ...
        try:
            ...
            with dynesty.pool.Pool(npool, loglike_func, prior_func) as pool:
                ...
                dsampler.run_nested(
                                ...
                                checkpoint_file=f'{out_basename}dynesty.save',
                                resume=resume,
                            )
                
        except AttributeError as e:
            try:
                dsampler = dynesty.NestedSampler(loglike_func, prior_func, 
                                    len(self.parameters), 
                                    nlive=n_live_points,
                                    sample=sample_method
                                    )
